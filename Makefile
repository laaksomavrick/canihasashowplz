...
.PHONY: build-prod
build-prod:
	@sam build -t $(TEMPLATE_FILE) --config-file $(CONFIG_FILE) --config-env prod --parameter-overrides ModelInferenceVersion=$(MODEL_INFERENCE_VERSION) ModelDataVersion=$(MODEL_DATA_VERSION) ModelTrainingVersion=$(MODEL_TRAINING_VERSION)
